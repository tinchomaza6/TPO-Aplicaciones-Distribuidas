...
<body>
	...
	<h2 class="text-center">Pedidos aprobados en espera de despacho</h2>
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td class="active">
						Calle: 
						<%= pedido.getCalleDireccEnvio() %>
						Numero:
						<%= pedido.getNroDireccEnvio() %>
						Localidad:
						<%= pedido.getLocalidadDireccEnvio() %>
						Cod. Postal: 
						<%= pedido.getCpDirecEnvio() %>
						</td>
						<td class="active">
						<button class="btn btn-success btn-xs" type="button" onclick="location.href='Inicio?action=despacharPedido&idPedido=<%=pedido.getNroPedido()%>'">Despachar</button>
						</td>
					</tr>
					<%
							}
